...
vector<pair<pair<int,int>,char>> dist = {{{0 , 1} , 'R'} , {{1 , 0} , 'D'} , {{-1 , 0} , 'U'} , {{0 , -1} , 'L'} };
int n , m;
void bfs01(vector<vector<char>> &grid , vector<vector<int>> &dis){
   ...
   while(!q.empty()){
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(dist[i].second == dir){
                wt = 0;
            }

            int dx = x + dist[i].first.first;
            int dy = y + dist[i].first.second;

            if(dx < n && dy < m && dx >= 0 && dy >= 0){
                if(dis[dx][dy] > dis[x][y] + wt){
                    dis[dx][dy] = dis[x][y] + wt;
